...
class Board:
    ...
    def animate_piece_drop(self, row, col, player, window):
        # Start the piece at the top of the column
        drop_position = 0
        frame = 0
        while drop_position < row:
            # Clear the area
            self.draw(window, frame)
 
            cpos = col * CELL_SIZE + X_OFFSET + CELL_SIZE // 2 - 16
            rpos = drop_position * CELL_SIZE + Y_OFFSET + CELL_SIZE // 2 - 16
            if player == 1:
        ...
        # After the animation, finalize the piece's position
        self.add_piece(row, col, player)
...
BUTTON_WIDTH = 100
BUTTON_HEIGHT = 50
BUTTON_X = X_OFFSET + 700
BUTTON_Y = Y_OFFSET + 50
BUTTTON_COLOUR = (200,200,200)
BUTTON_HOVER_COLOUR = (150,150,150)
BUTTON_TEXT_COLOUR = (0,0,0)

def draw_button(window, colour, x, y, width, height, text = ''):
    pygame.draw.rect(window, colour, (x, y, width, height))
    if text != '':
        font = pygame.font.Font(None, 36)
        text_surface = font.render(text, True, BUTTON_TEXT_COLOUR)
...
while running:
    ...
    if not has_winner:
        if overflowing:
            ...
            if make_move:
                board.animate_piece_drop(grid_row, grid_col, player_id[current_player], window)
    ...
    if BUTTON_X <= mouse_x <= BUTTON_X + BUTTON_WIDTH and BUTTON_Y <= mouse_y <= BUTTON_Y + BUTTON_HEIGHT:
        draw_button(window, BUTTON_HOVER_COLOUR, BUTTON_X, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 'UNDO')
    else:
        draw_button(window, BUTTTON_COLOUR, BUTTON_X, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, 'UNDO')
